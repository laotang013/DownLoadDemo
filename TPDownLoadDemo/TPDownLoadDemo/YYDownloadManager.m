...
@property(nonatomic,strong)AFURLSessionManager *sessionMamager;
@end
@implementation YYDownloadManager
+ (NSURLSessionConfiguration *)backgroundURLSession {
...
{
    static AFURLSessionManager *manamger = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manamger = [[AFURLSessionManager alloc]initWithSessionConfiguration:[self backgroundURLSession]];
    });
    return manamger;
}
-(void)downloadData:(NSNotification *)notification
{
    if ([notification.object isKindOfClass:[NSURLSessionDownloadTask class]]) {
        ...
        NSLog(@"urlHost:%@",urlHost);
        //判断是否有错误 有错误说明是下载被中断了，所以我们通过key:NSURLSessionDownloadTaskResumeData去获取续传时候的data为什么通过这个呢。
...
//    NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier:url];
    AFURLSessionManager *manager = [self shareSessionManager];
